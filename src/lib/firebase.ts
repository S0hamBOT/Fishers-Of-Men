...
type ProblemData = { [key: string]: Problem };
...
export const saveProblemProgress = async (
  ...
  try {
    ...
    const userData = userDoc.data()!;
    let problemProgress = userData.problemProgress || {};
    problemProgress = { ...problemProgress, [problemId]: { completed } };
    ...
    if (completed) {
      if (lastSolvedDate) {
        lastSolvedDate.setUTCHours(0, 0, 0, 0);
        const daysDiff =
          (today.getTime() - lastSolvedDate.getTime()) / (1000 * 3600 * 24);

        metrics.currentStreak = daysDiff === 1 ? metrics.currentStreak + 1 : 1;
      } else {
        metrics.currentStreak = 1;
      }
      ...
      await updateDoc(userDocRef, {
        "metrics.problemsSolved": arrayRemove(problemId),
      });
    }
...
export async function calculateUserRank(userId: string): Promise<number> {
  ...
  userScores.sort((a, b) => b.problems - a.problems);
  return userScores.findIndex((score) => score.id === userId) + 1;
}
